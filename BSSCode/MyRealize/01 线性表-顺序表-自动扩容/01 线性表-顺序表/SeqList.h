...
////////////////////////////////////////////////////////
//函数声明
static bool InCapacity(SeqList* SList);
...
//函数实现接口
/////////////////////////////////////////////////////////////
//扩容  扩容陈成功 true 否则false
static bool InCapacity(SeqList* SList){
	ElemType* NewData = (ElemType*)realloc(SList->data, sizeof(ElemType)*SList->length * 2);
	if (NewData == NULL){
		return false;
	}
	SList->data = NewData;
...
void SeqListPush_back(SeqList* SList,ElemType val){
	...
	assert(SList->data != NULL);
	//空间满了并扩容失败
	if (IsFull(SList) && !InCapacity(SList)){
		printf("顺序表满了，扩容失败，无法插入！\n");
		return;
	}
...
void SeqListInsertByPos(SeqList* SList, int pos, ElemType val){
	assert(SList->data != NULL);
	if (IsFull(SList) || pos < 1 || pos > SList->length && !InCapacity(SList)){
		printf("不满足插入条件，插入失败！\n");
		return;
	}
